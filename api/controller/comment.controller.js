...
const commentController = {
    ...
    editComment: async (req, res, next) => {
        try {
            ...
            if (comment.userId !== req.user.id && !req.use.isAdmin) return next(errorHandler(403, 'you are not allow to edit this comment'));
    ...
    deleteComment: async (req, res, next) => {
        try {
            ...
            if (!comment) return next(errorHandler(404, 'comment not found'));
            if (comment.userId !== req.user.id && !req.use.isAdmin) return next(errorHandler(403, 'you are not allow to delete this comment'));
